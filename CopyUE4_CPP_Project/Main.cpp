//********* This version uses vert & horiz Scrollbars
// Chks SLN file selected.
// Copies WHOLE folder / sub-folders (except for Debug/Release/ipch/.vs)
// All files except SDF files copied
...
//	vector <string> lines;
	...
	if (solFile) {
		while (!solFile.eof()) {	//process text file lines
			...
			tmp = ln;
//			if ((int)tmp.find("GameMode") < 0) {	//don't change GameMode lines!
			//	string os = oldS, ns = newS;
			//	string gm = os + "GameMode.";
			//	int pos = (int)tmp.find(gm);
			//	if ( pos > 0) {
			//		tmp = tmp.substr(0, pos) + ns + tmp.substr(pos + os.length(), tmp.length() - pos - os.length());
			//	}
			//}
			//else {
				replaceAll(tmp, oldS, newS);	//replace old strs to new
				if (chgUpperCase)
					replaceAll(tmp, u1, u2); //replace UPPER case old strs to new
//			}
			lines.push_back(tmp);
		}
		tmp = sfn;
...
//	strcpy_s(t, currDir);	//currDir 'loses' string!!! ADDED for Debug version!
	files.clear();
	dirs.clear();
